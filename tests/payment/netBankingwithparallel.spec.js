...
const BASE_URL = 'https://staging.vfs.welcomecure.online/in-chandigarhuk?lang=en-US&dir=ltr';
...
// Function to generate mobile numbers (8-digit for registration, 10-digit for Razorpay)
...
// ✅ Changed to test.describe.parallel for concurrent test execution
test.describe.parallel('NetBanking Payment Functional Test', () => {
  ...
  for (let i = 0; i < NUMBER_OF_RUNS; i++) {
    test(`Complete registration and payment with NetBanking - Run ${i + 1}`, async ({ page }) => {
      ...
      await registerPage.enterMobile(testData.mobile);           // 8 digits
      ...
      await razorpayPage.completeNetBankingPayment(testData.mobileRazorpay, true);  // 10 digits
